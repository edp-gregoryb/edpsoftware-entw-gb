...
@Injectable()
export class DelterminService {
loginparameter:any;
  constructor(private http: HttpClient, private loginparameterService:LoginparameterService) {
 this.loginparameter = this.loginparameterService.getparameter();
   }
    delTermin(rowid) {
    console.log("delTermin",rowid);
    let body = '{"request":{"filter":"restitdeletetermin%04Termid%05'+this.loginparameter.logintermid+'%06sprache%05'+this.loginparameter.loginsprache+'%06rowid%05'+rowid+
    ...
    let jsonbase64 = this.loginparameterService.stringtobase64(jsondata);
     //let body = '{"request":{"filter":"restitdeletetermin%04'+jsonbase64+'"}}';
